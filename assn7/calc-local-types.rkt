...
(define (calc-locals (e : ExprS) (knownNames : (listof symbol))) : (listof symbol)  
  (type-case ExprS e
    ...
    ;;; record operators
    ; create a record
    [recordS (fields) (append (map (λ (field) (fieldS-name field)) fields)
                              knownNames)]
    ; retrieve field value from a record
    [lookupS (rec name) (calc-locals rec knownNames)]
    ; add a field to a record
    [extendS (rec name val) (cons name (calc-locals val knownNames))]
    ; make record names available in the body (the Javascript with statement)
    [openS (rec recType body)
           (if (recT? recType)               
               (calc-locals body (append (map (λ (fieldType) (fieldT-name fieldType)) (recT-fieldTypes recType))
                                         knownNames))
               (error 'calc-locals "rec and something else"))]
    ))
