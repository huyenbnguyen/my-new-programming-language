           ...
           [(cons? cs)
            (let ([l (eqCon-lhs (first cs))]
              ...
              (cond
                ...
                [(is-identifier l) (unify/theta (replace-in-constraints l r (rest cs))
                                                (cons (first cs) (replace-in-constraints l r theta)))]
                [(is-identifier r) (unify/theta (replace-in-constraints r l (rest cs))
...
(define (occurs-check [l : Term] [r : Term]) : boolean
  (cond
    ((and (tArrow? l) (tArrow? r)) (if (or (equal? (tArrow-dom l) r)
                                           (equal? (tArrow-dom r) l)
                                           (equal? (tArrow-rng l) r)
                                           (equal? (tArrow-rng r) l))
                                       (error 'occurs-check "occurs-check")
                                       false))
    ((and (tArrow? l) (tList? r)) (if (or (equal? (tArrow-dom l) r)
                                          (equal? (tArrow-rng l) r))
                                      (error 'occurs-check "occurs-check")
                                      false))
    ((and (tList? l) (tArrow? r)) (if (or (equal? (tArrow-dom r) l)
                                          (equal? (tArrow-rng r) l))
                                      (error 'occurs-check "occurs-check")
                                      false))
    ((and (tList? l) (tList? r)) (if (or (equal? l (tList-elem r))
                                         (equal? r (tList-elem l)))                                     
                                     (error 'occurs-check "occurs-check")
                                     false))
    (else false)))
